...
import { dispatchHookEvent, type DispatchRequest } from "../src/core/dispatcher.js";
import type { HookExecutionResult } from "../src/core/output.js";
...
describe("dispatcher core", () => {
  ...
  it("executes selected entries and returns combined output", async () => {
    ...
    const output = await dispatchHookEvent({
      ...
      envelope: makeEnvelope("PreToolUse"),
      entries,
      execute: async (entry: RegistryEntry) => {
  ...
  it("uses entry timeout when isolating hook execution failures", async () => {
    const output = await dispatchHookEvent({
      ...
      execute: () => new Promise<HookExecutionResult>(() => {})
    });
  ...
  it("ignores late rejections after timeout-shaped fail-closed output", async () => {
    ...
    try {
      const output = await dispatchHookEvent({
        ...
        execute: () => new Promise<HookExecutionResult>((_resolve, reject) => {
  ...
  async function dispatchFailingRequest(eventName: SupportedEventName, entry: RegistryEntry) {
...
function makeEnvelope(eventName: SupportedEventName): HookEnvelope {
  return {
    eventName,
    sessionId: "session-1",
    cwd: "/workspace",
