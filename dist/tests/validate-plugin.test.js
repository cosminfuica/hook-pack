...
function createFixture(options = {}) {
    ...
    writeFileSync(resolve(root, "dist", "src", "core", "registry.js"), options.registrySource ?? "export const BUILT_IN_REGISTRY = [];\n", "utf8");
    return root;
}
...
describe("plugin validator", () => {
    ...
    it("fails on unquoted dispatch command and wrong timeout", async () => {
        await withFixture({
            ...
            assert.notEqual(result.exitCode, 0);
            assert.equal(result.stdout, "");
            assert.match(result.stderr, /^- PreToolUse registration 0 hook 0 command must include "\$\{CLAUDE_PLUGIN_ROOT\}\/hooks\/dispatch\.sh"/m);
    ...
    it("fails when implemented registry IDs lack stable ID governance docs", async () => {
        await withFixture({
            ...
            assert.notEqual(result.exitCode, 0);
            assert.equal(result.stdout, "");
            assert.match(result.stderr, /^- migration governance docs must exist when registry IDs are implemented at docs\/architecture\/migration-governance\.md/m);
        });
    });
});
