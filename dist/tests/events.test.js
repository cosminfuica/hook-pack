...
import { EVENT_DEFINITIONS, HookInputError, REGISTERED_DISPATCH_EVENTS, SUPPORTED_EVENTS, normalizeHookInput } from "../src/core/events.js";
describe("event adapter contracts", () => {
    ...
    function findDefinition(eventName) {
    ...
    it("defines event metadata as iterable definitions", () => {
        ...
        for (const definition of EVENT_DEFINITIONS) {
            ...
            assert.deepEqual(definition.outputCapabilities.filter((capability) => staleMetadataValues.includes(capability)), [], `${definition.name} must not use stale output capability names`);
        }
    });
    ...
    it("rejects every tool-matched event without tool_name", () => {
        for (const eventName of toolMatchedEvents) {
            assert.throws(() => normalizeHookInput({
                cwd: "/project",
                hook_event_name: eventName
            }, eventName), HookInputError, `${eventName} must reject missing tool_name`);
            assert.throws(() => normalizeHookInput({
                cwd: "/project",
                hook_event_name: eventName,
                tool_name: "  "
            }, eventName), HookInputError, `${eventName} must reject blank tool_name`);
        }
    });
    ...
    it("rejects mismatched expected event names", () => {
        assert.throws(() => normalizeHookInput({
            ...
            tool_name: "Write"
        }, "PreToolUse"), HookInputError);
    });
    it("rejects missing cwd", () => {
        assert.throws(() => normalizeHookInput({
            hook_event_name: "UserPromptSubmit",
            user_prompt: "Add tests"
        }, "UserPromptSubmit"), /cwd must be a string/);
    });
});
